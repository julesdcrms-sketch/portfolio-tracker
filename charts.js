...
function buildPie(){
  ...
  ].map(x=>`<div style="display:flex;align-items:center;gap:8px;font-size:13px">
  ...
  pieChart=new Chart(document.getElementById('pie'),{
    ...
    options:{responsive:false,cutout:'68%',plugins:{legend:{display:false},tooltip:{backgroundColor:'#1a1a18',titleColor:'#e8e6de',bodyColor:'#9a9890',callbacks:{label:ctx=>` ${ctx.parsed.toFixed(1)}%`}}}}
  });
}
...
function buildGeoMap(){
  ...
  wrap.innerHTML='';
  const W=wrap.offsetWidth||640,H=220;
  ...
  d3.json('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json').then(world=>{
    ...
    wrap.innerHTML='<div style="height:220px;display:flex;align-items:center;justify-content:center;font-size:11px;color:var(--text4)">Carte non disponible hors connexion</div>';
  });
